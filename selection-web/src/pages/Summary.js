  ...
  useEffect(() => {
    const fetchData = async id => {
      try {
        ...
        setReport(report);
        let array = [];
        response.Candidates.map(candidate => array.push(candidate.voteAmount));
        console.log(array);
        ...
        const max = Math.max.apply(
          Math,
          response.Candidates.map(function(o) {
            return o.voteAmount;
          })
        );
        setMaxValue(max);
            ...
            <Item.Group divided>
              {campaign &&
                campaign.map(campaign => (
                  <Item>
                    <Item.Image size="small" src={campaign.imageURL} />
                    <Item.Content
                    ...
                    >
                      ...
                      <Item.Description style={{ marginTop: "1em" }}>
                        <Progress
                          ...
                          total={sum}
                          color={
                            campaign.voteAmount === maxValue ? "green" : "grey"
                          }
                        />
                      </Item.Description>
                    </Item.Content>
